...
# ── Commands ────────────────────────────────
@app.on_message(filters.command(["start", "help", "shop", "orders", "balance", "addbalance", "profile"]) & filters.private)
async def commands_h(client, message):
    ...
    elif cmd in ["balance", "profile"]: await profile_menu(client, message)
...
async def admin_cmds(client, message: Message):
    ...
    elif cmd in ["setfsub", "setupi", "recovery", "fa2"]: await set_config_cmd(client, message)
...
async def msg_h(client, message):
    ...
    if message.from_user.id in payment_admin_states: await handle_admin_rejection_reason(client, message)
    elif message.from_user.id in session_states: await handle_session_message(client, message)
