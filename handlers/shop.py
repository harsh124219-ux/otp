...
async def view_country_accounts(client: Client, callback: CallbackQuery):
    ...
    accounts = get_available_accounts(country) # Already sorted by price descending
    
    if not accounts:
        await callback.answer("No accounts available for this country.", show_alert=True)
        return

    text = f"🛒 **ACCOUNTS IN {country}**\n\nSorted by: Expensive ➔ Cheap\n\n"
    buttons = []
    for acc in accounts:
        masked = mask_number(acc["phone"])
        buttons.append([InlineKeyboardButton(f"{masked} - ₹{acc['price']}", callback_data=f"buy_acc_{acc['phone']}")])
...
async def get_otp_logic(client: Client, callback: CallbackQuery):
    ...
    if not order or order["status"] == "closed":
        await callback.answer("Order inactive.", show_alert=True)
        return
    ...
    try:
        ...
        async for m in user_client.get_chat_history(777000, limit=1):
            otp = m.text if m.text else "Non-text message."
        await user_client.disconnect()
        
        await callback.message.reply_text(
            f"📩 **OTP FOR {order['phone']}**\n\n`{otp}`",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔄 Refresh", callback_data=f"get_otp_{order_id}")],
                [InlineKeyboardButton("🚪 Logout", callback_data=f"logout_acc_{order_id}")]
            ])
        )
...
async def logout_acc_logic(client: Client, callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(f"✅ Logged out from {order['phone']}.")
    except:
        await callback.answer("Error logging out.", show_alert=True)
